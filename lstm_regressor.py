...
INPUT_SIZE = 1
OUTPUT_SIZE = 1
CELL_SIZE = 10  # CELL_SIZE = #hidden_units in the cell
LR = 0.006
BATCH_START_TEST = 0
...
def get_batch():
    ...
    seq = np.sin(xs) # sample data
    res = np.cos(xs) # result data or label data
...
if __name__ == '__main__':
    ...
    # plt.show()
    for i in range(1000):
        seq, res, xs = get_batch()  # get created data
        if i==0:
            feed_dict = {
                model.xs: seq,
                model.ys: res,
